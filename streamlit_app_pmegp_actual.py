"""
PMEGP Executive Dashboard - ULTIMATE VERSION (Actual Field Structure)
=====================================================================

Enhanced Streamlit dashboard with:
- All actual PMEGP fields from the division dataset
- Interactive visualizations & heatmaps
- Comprehensive KPI analysis
- Advanced clustering & profiling
...
def load_data(csv_path):
  ...
  try:
    ...
    # Force numeric types for ALL financial columns BEFORE any math
    num_cols = [
      'EMPLOYMENT_AT_SETUP',
      'PROJ_COST',
...
def create_state_performance_map(df):
    """Enhanced state performance map"""
    if len(df) == 0:
        return None
...
def create_financial_analysis(df):
  ...
  # Ensure all math columns are numeric for THIS filtered df
  num_cols = [
    'EMPLOYMENT_AT_SETUP',
    'PROJ_COST',
    'MARGIN_MONEY_SUBSIDY_RS',
  ...
  with col2:
    if all(col in df.columns for col in ['PROJ_COST', 'MARGIN_MONEY_SUBSIDY_RS', 'ANNUAL_TURNOVER']):
      ...
      # Prevent str/int division by ensuring numeric
      for c in ['PROJ_COST', 'MARGIN_MONEY_SUBSIDY_RS', 'ANNUAL_TURNOVER']:
...
def create_clustering_analysis(df):
    ...
    try:
        cluster_features = ['EMPLOYMENT_AT_SETUP', 'PROJ_COST', 
...
def main():
    ...
    filters = {
        'states': st.sidebar.multiselect('States', sorted(df['STATE_NM'].unique()), 
                                        default=sorted(df['STATE_NM'].unique())[:5]),
        'activities': st.sidebar.multiselect('Activities', sorted(df['ACTIVITY_NAME'].unique()),
                                            default=sorted(df['ACTIVITY_NAME'].unique())[:3]),
        'categories': st.sidebar.multiselect('Category', sorted(df['BENF_CATEGORY_DESC'].unique()),
                                            default=sorted(df['BENF_CATEGORY_DESC'].unique())),
        'status': st.sidebar.multiselect('Status', sorted(df['CURRENT_STATUS'].unique()),
                                        default=sorted(df['CURRENT_STATUS'].unique())),
        'location': st.sidebar.multiselect('Location', sorted(df['IND_TYPE'].unique()),
                                          default=sorted(df['IND_TYPE'].unique()))
    }
